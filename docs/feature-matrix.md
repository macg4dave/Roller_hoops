...
| UI metadata editing | Edit `owner`/`location`/`notes` after creation | ui-node | (calls Go API) | none (no DB access) | planned |
| Authentication & sessions | Local users + session cookies in UI | ui-node | (N/A) | none | planned |
| Protect `/api` | Prevent browser-direct access to Go API (UI-as-BFF) | traefik + ui-node | `/api/...` (internal only) | none | complete |
