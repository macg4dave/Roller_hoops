...
| UI polish & accessibility (Phase 12) | Keyboard navigation, focus styles, contrast, reduced motion, and resilient polling/loading states across the UI | ui-node | (calls Go API) | none | complete |
| Network map UI shell | `/map` route with constant 3-pane layout (Layer panel / Canvas / Inspector), empty-by-default canvas, deep-linkable layer + focus in URL, and inspector-driven cross-layer navigation stubs. | ui-node | (calls Go API later) | none | in-progress |
