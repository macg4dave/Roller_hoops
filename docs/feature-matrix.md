...
| Basic focused network diagram | Device-focused map view showing an ordinary readable diagram: selected device, nearby router/switch/peer when known, visible IP/MAC facts, and relationship source/confidence. | core-go + ui-node | `/api/v1/map/{layer}` | `devices`, `ip_addresses`, `mac_addresses`, `interfaces`, `links` | planned |
