...
| Map projection API (base) | Projection-first read endpoints returning render-ready `regions[]/nodes[]/edges[]` + `inspector` for a focused object; **no global graph** endpoints. | core-go | `/api/v1/map/{layer}` (scaffolding; starting with `/api/v1/map/l3`) | (derived from existing tables; no new tables required for L3 v1) | in-progress |
| Map projection: L3 (Subnets) | Subnet regions derived from IP facts; **device + subnet focus are live** (no global graphs). | core-go + ui-node | `/api/v1/map/l3` | `ip_addresses`, `devices` (+ observations later) | in-progress |
