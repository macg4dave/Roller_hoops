...
|---|---|---|---|---|---|
| Devices API | CRUD for devices (headless) | core-go | `/api/v1/devices` | `devices` | in-progress |
...
| Reverse proxy routing | `/` → UI, `/api` → Go | traefik | (N/A) | none | in-progress |
| Docker compose bootstrap | `docker compose up` works with health checks | (all) | (N/A) | (all) | in-progress |
