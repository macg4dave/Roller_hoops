...
| Data retention cleanup policy | Documented retention windows and manual cleanup SQL for append-only history/operational tables, backed by retention-friendly indexes. | core-go + operations | (N/A) | `ip_observations`, `mac_observations`, `discovery_run_logs`, `audit_events` | complete |
...
| Basic focused network diagram | Device-focused map view showing an ordinary readable diagram: selected device, nearby router/switch/peer when known, visible IP/MAC facts, and relationship source/confidence. | core-go + ui-node | `/api/v1/map/{layer}` | `devices`, `ip_addresses`, `mac_addresses`, `interfaces`, `links` | complete |
...
| Map projection: L2 (VLANs) | VLAN regions and membership based on SNMP-derived VLAN facts (start with PVID). | core-go + ui-node | `/api/v1/map/l2` | `interface_vlans`, `interfaces`, `devices` | complete |
| Map projection: Physical | Physical adjacency projection based on curated/manual links initially, with future LLDP/CDP enrichment possible. | core-go + ui-node | `/api/v1/map/physical` | `links` | complete |
| Map projection: Services | Services view grouping by host from discovered services; optional manual dependencies as explicit edges. | core-go + ui-node | `/api/v1/map/services` | `services` (+ planned `service_dependencies`) | complete |
| Map projection: Security | Zones as regions with device occupants; no inter-zone policy edges in v1. | core-go + ui-node | `/api/v1/map/security` | `zones`, `device_zones` | complete |
| Map editing (Build mode) | Author curated truth (links/zones) through Go APIs; UI never touches DB directly. `service_dependencies` deferred. | core-go + ui-node | `/api/v1/topology/zones` (implemented, T016), `/api/v1/topology/links` (planned, T019) | `links`, `zones`, `device_zones` | in progress (zone CRUD live; link CRUD and UI controls pending) |
| Map Operate overlays | Read-only operational overlays for active map projections: device recency, recent changes, region rollups, legend, and compact Inspector history. | core-go + ui-node | `/api/v1/map/{layer}`, `/api/v1/devices/changes`, `/api/v1/devices/{id}/history` | `ip_observations`, `mac_observations`, `services`, `device_metadata`, `devices` | planned |
